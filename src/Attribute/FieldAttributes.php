...
trait FieldAttributes
{
    ...
    private function addValidatorAttributeHtml(
        ...
        /** @var array */
        $rules = $model->getRules()[$attribute] ?? [];
    ...
    private function getId(): string
    {
        ...
        return $id === '' ? HtmlModel::getInputId($new->getModel(), $new->attribute) : $id;
    }
    ...
    private function setInputAttributes(array $attributes): array
    {
        ...
        $attributes = $new->addValidatorAttributeHtml($new->getModel(), $new->attribute, $attributes, $type);
        $attributeName = HtmlModel::getAttributeName($new->getModel(), $new->attribute);
        $attributeValue = HtmlModel::getAttributeValue($new->getModel(), $new->attribute);
        ...
        if ($new->getModel()->hasErrors($attributeName) && $new->errorClass !== '') {
            Html::addCssClass($attributes, $new->invalidClass);
        } elseif (!empty($attributeValue) && $new->validClass !== '') {
            Html::addCssClass($attributes, $new->validClass);
        }

        if (!in_array($type, self::NO_PLACEHOLDER_TYPES, true)) {
            $placeHolder = $new->getModel()->getAttributePlaceholder($new->attribute);
        }
